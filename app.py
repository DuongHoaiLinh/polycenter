...
from models_db import db, Student, Class, Course, Teacher, Enrollment, ClassSchedule, User, TestResult
...
def get_student_scores(student_id):
    ...
    # Kiểm tra xem người dùng hiện tại có quyền xem điểm của student_id không
    if user_data.get('role') != 'admin':
        linked_id = user_data.get('linked_id')
        if linked_id is None or user_data.get('role') != 'student' or int(linked_id) != student_id:
            return jsonify({'error': 'Không có quyền xem điểm này'}), 403
    
    student = Student.query.get(student_id)
    if not student:
        return jsonify({'error': 'Không tìm thấy thông tin học viên'}), 404
    
    scores = TestResult.query.filter_by(student_id=student_id).all()
    score_data = []
    ...
    for score in scores:
        score_item = {
            ...
            'status': 'Đạt' if score.score >= 5 else 'Không Đạt'
        }
...
def register():
    ...
    try:
        ...
        phone_exists = Student.query.filter_by(phone=phone).first()
        if email_exists or phone_exists:
            return "Email hoặc số điện thoại đã được sử dụng!", 400

        # Tạo mới học viên
        new_student = Student(name=name, email=email, phone=phone)
        db.session.add(new_student)
        db.session.flush()  # Lấy ID của student mới
